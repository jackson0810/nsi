...
$Id: CachingListener.cfc 1546 2009-05-12 21:57:07Z peterfarrell $
...
<cfcomponent
	...
	<cffunction name="reapAll" access="public" returntype="void" output="false"
		...
		<cfset var module = "" />
		<cfset var cacheStrategyName = "" />
		<cfset var message = CreateObject("component", "MachIIDashboard.model.sys.Message").init("Reaped all cache strategies.") />
	...
	<cffunction name="flushAll" access="public" returntype="void" output="false"
		...
		<cfset var module = "" />
		<cfset var cacheStrategyName = "" />
		<cfset var message = CreateObject("component", "MachIIDashboard.model.sys.Message").init("Flushed all cache strategies.") />
	...
	<cffunction name="reapCacheStrategy" access="public" returntype="void" output="false"
		...
		<cfset var message = CreateObject("component", "MachIIDashboard.model.sys.Message").init("Reaped '#strategyName#' in module '#arguments.event.getArg("ModuleName")#'.") />		
	...
	<cffunction name="flushCacheStrategy" access="public" returntype="void" output="false"
		...
		<cfset var strategyName = arguments.event.getArg("strategyName") />
		<cfset var cacheStrategy = getCacheStrategyByModuleAndStrategyName(arguments.event.getArg("moduleName"), strategyName) />
		<cfset var message = CreateObject("component", "MachIIDashboard.model.sys.Message").init("Flushed '#strategyName#' in module '#arguments.event.getArg("ModuleName")#'.") />		
	...
	<cffunction name="getModuleCacheStrategies" access="private" returntype="struct" output="false"
		...
		<cfloop collection="#modules#" item="key">
			...
			<cfif ArrayLen(cacheStrategyNames)>
				<cfset cacheStrategies[key] = structNew() />
				<cfloop from="1" to="#ArrayLen(cacheStrategyNames)#" index="i">
					<cfset cacheStrategies[key][cacheStrategyNames[i]] = cacheStrategyManager.getCacheStrategyByName(cacheStrategyNames[i]) />
				</cfloop>
			</cfif>
		</cfloop>
